...
export function HospitalTab() {
  ...
  const [hospitalFromDate, setHospitalFromDate] = useState<Date>(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
  ...
  const { data: hospitalReportsData, refetch } = useGetHospitalsReportsQuery({
    startDate: format(hospitalFromDate, "yyyy-MM-dd"),
    endDate: format(hospitalToDate, "yyyy-MM-dd"),
          ...
          <Select value={selectedHospital} onValueChange={setSelectedHospital}>
            ...
            <SelectContent>
              ...
              {hospitalOptions.map((hospital) => (
                <SelectItem key={hospital as string} value={(hospital as string).toLowerCase().split(" ").join("")}>
                  {hospital as string}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-full justify-start text-left">
                ...
                {hospitalFromDate ? format(hospitalFromDate, "PPP") : "Select date"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0">
              <Calendar disabled={(date) => {
                ...
                return d > today;
              }} required mode="single" selected={hospitalFromDate} onSelect={setHospitalFromDate} initialFocus />
            </PopoverContent>
          </Popover>
        </div>
        <div className="space-y-2">
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0">
              <Calendar disabled={(date) => {
                ...
                const today = new Date(new Date().toDateString());
                return d > today;
              }} required mode="single" selected={hospitalToDate} onSelect={setHospitalToDate} initialFocus />
            </PopoverContent>
          </Popover>
        </div>
      </div>
      <Button onClick={handleGenerateHospitalReports} className="bg-[#3498db] hover:bg-[#2980b9] text-white">
        Generate Reports
      </Button>
      ...
      {showHospitalReports && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-[#2c3e50]">Hospital Reports</h3>
